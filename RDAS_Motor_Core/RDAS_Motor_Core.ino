...
void setup() {
  ...
  // -- tilt servos, depending on battery - either enable or disable at start -- //
...
void received_action(char action, char cmd, uint8_t key, uint16_t val, char delim) {
  ...
  if(cmd == 'T') { // tilt mode
    ...
    // using this button to now control the claw
    ...
    // bunch of old tilt mode code that is sometimes tempermental
